...
- **Multi-monitor Selection:** Choose which connected display the Timer Beam should appear on, with automatic fallback if a monitor is disconnected.
...
- **LivePreview Settings:** Real-time visual feedback on the actual Timer Beam as settings are adjusted, with explicit Save/Cancel persistence.
- **Deep macOS Integration:** Support for Launch at Login, multi-display awareness (targeting specific monitors), and persistent settings via UserDefaults.
