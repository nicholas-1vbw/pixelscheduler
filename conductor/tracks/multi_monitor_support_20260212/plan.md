...
- [ ] Task: Update `SettingsManager` to include `selectedDisplayName` (defaulting to empty).
...
- [ ] Task: Update `BeamWindow.update` and `FrameCalculator.calculateFrame` to explicitly use the `NSScreen` resolved from settings instead of defaulting to `.main`.
